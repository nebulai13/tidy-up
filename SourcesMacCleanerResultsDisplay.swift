...
struct ResultsDisplay {
    static func show(_ results: ScanResults) {
        ...
        for (index, file) in results.largeFiles.prefix(20).enumerated() {
            ...
            print("\(number). \(type) \(size) - \(file.path)")
            
            // Show age
            let daysSinceModified = Calendar.current.dateComponents([.day], from: file.lastModified, to: Date()).day ?? 0
            if daysSinceModified > 365 {
                let years = daysSinceModified / 365
                print("    ⏰ Last modified \(years) year\(years > 1 ? "s" : "") ago")
            } else if daysSinceModified > 30 {
                let months = daysSinceModified / 30
                print("    ⏰ Last modified \(months) month\(months > 1 ? "s" : "") ago")
            } else if daysSinceModified > 0 {
                print("    ⏰ Last modified \(daysSinceModified) day\(daysSinceModified > 1 ? "s" : "") ago")
            }
    ...
    static func interactiveMode(_ results: ScanResults) {
        ...
        let (storageAvailable, fastStorageAvailable) = Task {
            await storageManager.checkVolumesAvailable()
        }.result
        
        guard let volumes = try? storageAvailable.get() else {
            print("⚠️  Could not check volume availability")
            return
        }
        
        if !volumes.storage {
            print("⚠️  Storage drive not available at /Volumes/storage1/")
        }
        if !volumes.fastStorage {
            print("⚠️  Fast storage not available at /Volumes/flash1/")
        }
        ...
        for (index, file) in results.largeFiles.enumerated() {
            ...
            print("\(file.isDirectory ? "📁" : "📄") \(file.path)")
            print("Size: \(file.sizeFormatted)")
            print("Last modified: \(formatDate(file.lastModified))")
            print()
            
            print("Options:")
            print("  [s] Move to Storage (/Volumes/storage1/)")
            print("  [f] Move to Fast Storage (/Volumes/flash1/)")
            print("  [d] Delete")
            print("  [k] Keep (skip)")
            print("  [q] Quit")
            print()
